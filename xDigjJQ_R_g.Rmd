```{r}
# https://youtu.be/xDigjJQ_R_g

# 今回は以下に示しています3つの単位根検定(*)モデルのうちdriftモデルのtau検定統計量を確認します。
# *ここではADF検定とします。
# 参考引用文献
...
```{r drift-tau}
# 以下に示していますdriftモデルのtau検定統計量を求めます。
# ラグ次数pを4、ドリフト項あり、トレンド項なしとします。
p <- 4
result <- fun_test_statistic_of_adf(y = y,p = p,drift = T,trend = F)
...
# tau検定統計量はt値、-1.678244 です。

# Rの関数を利用した結果と照合します。
# CADFtest {CADFtest}
CADFtest(model = y,type = 'drift',max.lag.y = p)$statistic

# ur.df {urca}
ur.df(y = y,type = 'drift',lags = p) %>% summary()

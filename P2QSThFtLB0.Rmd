...
# ラグ次数pを3とした場合
CADFtest(model = y,type = 'drift',max.lag.y = 3)$statistic
result_adf <- fun_test_statistic_of_adf(y = y,p = 3,drift = T,trend = F,without_ylag1_term = F)
result_adf$adf_model
result_adf$result_lm %>% summary() %>% {.$coef[2,]}
# 検定統計量は同一です。

# ラグ次数pを0とした場合
CADFtest(model = y,type = 'drift',max.lag.y = 0)$statistic
result_adf <- fun_test_statistic_of_adf(y = y,p = 0,drift = T,trend = F,without_ylag1_term = F)
result_adf$adf_model
result_adf$result_lm %>% summary() %>% {.$coef[2,]}
# こちらも検定統計量は同一です。
```
...
# Dickey–Fullerテーブルの中段のdriftモデルのサンプルサイズ25と比較しますと、ほぼ同一です。
```
...
# Dickey–Fullerテーブルの中段のdriftモデルのサンプルサイズ250と比較しますと、ほぼ同一です。
